...
def predict_rotation_improved(
    ...
    meta = {
        ...
        "num_detections": {k: len([x for x in v[1] if x[2] >= 0.45]) 
                           for k, v in results.items()},
        "top_texts": {k: [x[1][:30] for x in v[1][:3] if x[2] >= 0.45]
                      for k, v in results.items()}
    }
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--in", dest="input_dir", required=False, default="debug_crops_3408",
                    help="Input folder containing images.")
    ap.add_argument("--out", dest="output_dir", required=False, default="out_folder_debug_crops_3408_rotated",
    ...
    ap.add_argument("--osd-weight", type=float, default=3.0,
                    help="Weight for Tesseract OSD bias (default: 3.0).")
    ap.add_argument("--use-osd-shortlist", action="store_true",
                    help="Use OSD to shortlist candidate angles (faster but may miss correct rotation).")
    ap.add_argument("--no-enhance-contrast", dest="enhance_contrast", action="store_false",
